...
import { submitRsvpToGoogleForm } from './useGoogleFormSubmit'
...
export function RsvpModal() {
  ...
  // Al abrir: pre-llenar con todos los invitados (todos asisten)
  useEffect(() => {
    if (!isOpen || !invitationList?.guests?.length) return
    setValue('names', [...invitationList.guests])
  }, [isOpen, invitationList?.guests, setValue])
  ...
  const onSubmit = (values: RsvpFormValues) => {
    submitRsvpToGoogleForm(values)
    reset({ names: [], attending: 'Sí', message: '' })
    closeModal()
  }
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Indicá por cada persona: nombre y si tiene alergias o restricciones alimentarias. Ej: Juan - sin restricciones; María - vegetariana, sin gluten."
                  />
                  <p className="mt-1 text-xs text-stone-500">
                    Especificá nombre de la persona y alergias o restricciones alimentarias.
                  </p>
                </div>
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    className="w-full py-3 px-4 bg-stone-800 text-white rounded-lg font-medium hover:bg-stone-700 transition"
                  >
                    Enviar y abrir formulario
                  </button>
                  <p className="mt-2 text-xs text-stone-500 text-center">
                    Serás redirigido a Google Forms para confirmar. Los datos ya estarán cargados.
                  </p>
                </div>
              </form>
            )}
